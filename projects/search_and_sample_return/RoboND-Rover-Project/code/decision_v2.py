...
def decision_step(Rover):
        ...
        if Rover.mode == 'forward': 
            ...
            if len(Rover.nav_angles) >= Rover.decelerate_forward:
                if np.sum(Rover.nav_angles_close[np.abs(Rover.nav_angles_close* 180/np.pi)<=30])>=Rover.obs_forward:  
                    ...
                    #Rover.steer = np.clip(steer_foward(Rover), -10, 10)
                else:
                    # If mode is forward, navigable terrain looks good 
                    # and velocity is below max, then throttle 
                    if Rover.vel < Rover.max_vel:
            ...
            # (len(Rover.nav_angles) < Rover.decelerate_forward) ---> decelerate
        ...
        # If we're already in "stop" mode then make different decisions
            ...
            # If we're not moving (vel < 0.2) then do something else
                ...
                if len(Rover.nav_angles) < Rover.go_forward:
                    ...
                    Rover.steer = -8 # Could be more clever here about which way to turn
                    #Rover.steer = random.randrange(-1, 2, 2) * 30
                ...
                if len(Rover.nav_angles) >= Rover.go_forward:
                    ...
                    #Rover.steer = np.clip(np.mean(Rover.nav_angles_close * 180/np.pi), -15, 15)
                    Rover.steer = steer_stop(Rover)
                    Rover.mode = 'forward'
    ...
    print(Rover.mode, len(Rover.nav_angles))
...
# *********************************************************************************************
# the 'steer_foward' function is not used.
